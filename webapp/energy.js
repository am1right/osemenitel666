...
const Energy = (() => {
    ...
    let   REGEN_MS    = 10 * 60 * 1000;  // 10 мин на 1% (база; сервер уточняет)
    ...
    const DRAIN_UNIT_MS = 10 * 1000;     // 1% за 10 секунд активной игры
    ...
    function showWidget(containerEl) {
        ...
        function refresh() {
            ...
            document.querySelectorAll('.js-continue').forEach((b) => {
                ...
                b.textContent = ok ? 'Продолжить · 100 choin' : '⚡ Купить заряд · 320 choin';
            });
        }
